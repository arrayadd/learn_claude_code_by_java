package learn.claude.code.common;
 ...
 * 所有课程共用这些工具，对应 Python 版中每个文件重复出现的工具函数。
 *
...
public class BaseTools {
    ...
    public String runBash(String command) {
        ...
        try {
            ...
            // 跨平台 shell 选择：Windows 用 cmd /c，Unix/Mac 用 sh -c
            // sh -c 可以执行包含管道、重定向等的复杂命令
            ...
            // 合并 stderr 到 stdout —— 简化输出处理，LLM 可以同时看到正常输出和错误信息
            pb.redirectErrorStream(true);

            Process proc = pb.start();
            // 读取命令输出 —— 必须在 waitFor 之前读取，否则缓冲区满了会导致死锁
            ...
            // 等待命令完成，超时 120 秒
            // 为什么是 120 秒？因为编译、测试等操作可能需要较长时间
            boolean finished = proc.waitFor(120, TimeUnit.SECONDS);
            if (!finished) {
                proc.destroyForcibly(); // 超时后强制终止进程
                return "Error: Timeout (120s)";
            }

            String result = output.toString().trim();
            if (result.isEmpty()) {
                // 返回 "(no output)" 而不是空字符串，让 LLM 知道命令成功执行但没有输出
                return "(no output)";
            }
            // 截断过长的输出 —— 防止占用过多 token 导致上下文溢出
            // 50000 字符约等于 12500 个 token，足够 LLM 理解命令结果
            return result.length() > 50000 ? result.substring(0, 50000) : result;

        } catch (Exception e) {
            // 返回错误信息而不是抛异常 —— 让 LLM 有机会理解错误并尝试修正
    ...
    public String runRead(String path, int limit) {
        try {
            ...
            // 字符级别的截断 —— 最后一道保护，防止超大文件消耗过多 token
            return result.length() > 50000 ? result.substring(0, 50000) : result;
        } catch (Exception e) {
            return "Error: " + e.getMessage();
        }
     ...
     *
     * 为什么要提供这个便捷方法？
     * 因为大多数课程都需要这 4 个基础工具，每次手动添加既冗余又容易遗漏。
     * 统一提供一个 allToolDefs() 方法，确保一致性。
     */
